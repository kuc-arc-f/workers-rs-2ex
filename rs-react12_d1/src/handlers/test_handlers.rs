...
pub async fn handle_list_todos(_: Request, ctx: RouteContext<()>) -> worker::Result<Response> {
    ...
    let query = "SELECT id, title, created_at, updated_at FROM todos ORDER BY created_at DESC";
...
pub async fn handle_create_todo(mut req: Request, ctx: RouteContext<()>) -> worker::Result<Response> {
    ...
    let query = "INSERT INTO todos (title, created_at, updated_at) VALUES (?, ?, ?) RETURNING id, title, created_at, updated_at";
    let stmt = db.prepare(query)
...
pub async fn handle_update_todo(mut req: Request, ctx: RouteContext<()>) -> worker::Result<Response> {
    ...
    let query = format!("UPDATE todos SET {} WHERE id = ? RETURNING id, title, created_at, updated_at", query_parts.join(", "));
